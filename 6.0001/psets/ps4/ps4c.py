...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for permutation in permutations:
            dic = self.build_transpose_dict(permutation)
            applied_text = self.apply_transpose(dic)
            applied_text_split = applied_text.split(' ')
            valid_word_count = 0
            for word in applied_text_split:
                if is_word(self.valid_words, word):
                    valid_word_count +=1
            attempts[valid_word_count]=applied_text
