    ...
    def build_shift_dict(self, shift):
        ...
        for character in text:
            if character in lowercase:
                dic[character] = lowercase[(lowercase.index(character)+shift)%26]
            elif character in uppercase:
                dic[character] = uppercase[(uppercase.index(character)+shift)%26]
        return dic
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        attempts = {}
        for i in range(26):
            test_text = Message.apply_shift(self, i)
            test_text_split = test_text.split(' ')
            valid_word_count = 0
            for word in test_text_split:
                if is_word(self.valid_words, word):
                    valid_word_count+=1
            attempts[valid_word_count]=(i,test_text)
