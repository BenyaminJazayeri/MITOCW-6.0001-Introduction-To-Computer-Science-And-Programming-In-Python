...
class PhraseTrigger(Trigger):
    ...
    def is_phrase_in(self,text):
        ...
        if phrase == clean_text or ' ' + phrase in clean_text or phrase + ' ' in clean_text:
            return True
        return False
...
def read_trigger_config(filename):
    ...
    for line in lines:
        ...
        if line_list[0] == 'ADD':
            line_list.remove('ADD')
        for trigger in line_list:
            triggerlist.append(triggerdict[trigger])
        else:
            if line[1] == "TITLE":
                triggerdict[line[0]] = TitleTrigger(line[2])
            if line[1] == "DESCRIPTION":
                triggerdict[line[0]] = DescriptionTrigger(Trigger(line[2]))
            if line[1] == "AFTER":
                triggerdict[line[0]] = AfterTrigger(Trigger(line[2]))
            if line[1] == "BEFORE":
                triggerdict[line[0]] = BeforeTrigger(Trigger(line[2]))
            if line[1] == "NOT":
