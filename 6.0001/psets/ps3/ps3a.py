...
def get_word_score(word, n):
    ...
    """
    word=word.lower()
    sum_of_letter_points = 0
    for letter in word:
        sum_of_letter_points+=SCRABBLE_LETTER_VALUES[letter]
    wordlen = len(word)
    component_2 = max((7*wordlen - 3*(n-wordlen)),1)
    return sum_of_letter_points*component_2
...
def is_valid_word(word, hand, word_list):
    ...
    returns: boolean
    """
    #double asterix is unchecked
    word = word.lower()
    hand_copy = hand.copy()
    asterix_index=word.find('*')
    ...
    if asterix_index>-1:
        for i in VOWELS:
            hand_temp = hand.copy()
            del(hand_temp['*'])
            hand_temp[i] = hand_temp.get(i,0)+1
            temp = list(word)
            temp[asterix_index]=i
            new_word=''.join(temp)
            result_list.append(is_valid_word(new_word,hand_temp,word_list))
...
def substitute_hand(hand, letter):
    ...
    if letter not in hand.keys():
        ...
        for l in hand.keys():
            if l in letter_choices:
                letter_choices.remove(l)
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while(number_of_hands>0):
        ...
        display_hand(hand)
        if not answerd_yes:
            que=input("Would you like to substitute a letter? ")
            que=que.lower()
            if que == 'yes':
                answerd_yes=True
        ...
        if not answerd_yes_2:
            answerd_yes_2=True
            que2=input("Would you like to substitute a letter? ")
            que2=que2.lower()
            if que2 == 'yes':
                answerd_yes_2=True
